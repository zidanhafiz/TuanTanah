...
pnpm typecheck            # tsc --noEmit across all workspaces (this is the only check — no lint, no tests yet)
pnpm build                # builds client to client/dist
pnpm redis                # docker compose -f docker-compose.dev.yml up -d redis
```

There is **no test runner and no linter** configured. `pnpm typecheck` is the sole automated gate; run it after changes. The engine is written to be pure and testable (RNG is injectable via the `Rng` param), so a test harness can be added later without refactoring.
